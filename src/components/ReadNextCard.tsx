    ...
    <ReadNextCardArticle className="read-next-card">
      ...
      <ReadNextCardContent className="read-next-card-content">
        <ul>
          {relatedPosts.map(n => {
            ...
            // 20 AUG 2018
            const displayDatetime = format(date, 'dd LLL yyyy', {locale: ru});
            return (
      ...
      <ReadNextCardFooter className="read-next-card-footer">
        <Link to={`/tags/${kebabCase(props.tags[0])}/`}>
          {props.relatedPosts.totalCount > 1 && `Смотреть все ${props.relatedPosts.totalCount} статьи`}
          {props.relatedPosts.totalCount === 1 && '1 статья'}
          {props.relatedPosts.totalCount === 0 && 'Нет статей'} →
        </Link>
      </ReadNextCardFooter>
    </ReadNextCardArticle>
  );
